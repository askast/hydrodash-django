...
def yi(pow_i, motorHP):
    return ((-0.4508*math.pow((pow_i/motorHP), 3))+(1.2399*math.pow((pow_i/motorHP), 2))-(0.4301*(pow_i/motorHP))+0.6410)
...
def calculatePEI(bep_flow, bep_head, bep_power, flow_75, head_75, power_75, flow_110, head_110, power_110, power_120, tempRPM, pump_type, test_type, motor_hp=0, motor_eff=0):
    ...
    specificSpeed = nomspeed * \
        math.pow(bep_flow_corr, 0.5)*math.pow(bep_head_corr, -0.75)
    ...
    motorHPs = [1, 1.5, 2, 3, 5, 7.5, 10, 15, 20, 25,
                30, 40, 50, 60, 75, 100, 125, 150, 200]
    motor_eff_1800 = [85.5, 86.5, 86.5, 89.5, 89.5, 91, 91.7, 92.4,
                      93, 93.6, 93.6, 94.1, 94.5, 95, 95, 95.4, 95.4, 95.8, 95.8]
    motor_eff_3600 = [77, 84, 85.5, 85.5, 86.5, 88.5, 89.5, 90.2,
                      91, 91.7, 91.7, 92.4, 93, 93.6, 93.6, 93.6, 94.1, 94.1, 95]
    ...
    STDEff = (-0.85*math.pow(math.log(bep_flow_corr), 2)-0.38*math.log(specificSpeed)*math.log(bep_flow_corr)-11.48 *
              math.pow(math.log(specificSpeed), 2)+17.80*math.log(bep_flow_corr)+179.80*math.log(specificSpeed)-(C_factor+555.6))/100
    ...
    motorLoss75 = motorLossFull*yi(hydropower_75/(STDEff*.95), motorHP)
    motorLoss110 = motorLossFull * \
        yi(hydropower_110/(STDEff*.985), motorHP)
    if test_type == "BP":
    ...
    PERstd = (hydropower_75/(0.95*STDEff)+motorLoss75)/3+(hydrobep_power /
                                                          STDEff+motorLossBEP)/3+(hydropower_110/(0.985*STDEff)+motorLoss110)/3
    PERcl = (drive_input_power_75)/3+(drive_input_power_bep) / \
        3+(drive_input_power_110)/3
    PEIcl = PERcl/PERstd

    flow_25_corr = 0.25*bep_flow_corr
    flow_50_corr = 0.5*bep_flow_corr
    pump_input_power_25 = (0.80*math.pow(flow_25_corr/bep_flow_corr, 3) +
                           0.20*(flow_25_corr/bep_flow_corr))*bep_power_corr
    pump_input_power_50 = (0.80*math.pow(flow_50_corr/bep_flow_corr, 3) +
                           0.20*(flow_50_corr/bep_flow_corr))*bep_power_corr
    pump_input_power_75 = (0.80*math.pow(flow_75_corr/bep_flow_corr, 3) +
                           0.20*(flow_75_corr/bep_flow_corr))*bep_power_corr
    ...
    PERvl = 0.25*drive_input_power_25+0.25*drive_input_power_50 + \
        0.25*drive_input_power_75+0.25*drive_input_power_bep
    PEIvl = PERvl/PERstd
