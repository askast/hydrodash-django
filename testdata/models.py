...
class RawTestsList(models.Model):
    testdatatype_choices = (('CE', 'Commercial Efficiency'),
                            ('CP', 'Commercial Standard Performance'), ('RS1', 'Residential Stand 1'), ('RS2', 'Residential Stand 2'), ('CO', 'Commercial Outside Stand'))

    headunits_choices = (('ft', 'Feet of water'), ('m', 'meters of water'))
    powerunits_choices = (
        ('W', 'Watts'), ('KW', 'Kilo Watts'), ('HP', 'Horsepower'))
    flowunits_choices = (('gpm', 'Gallons per minute'), ('m3ph',
                                                         'Cubic meter per hour'), ('lpm', 'liters per minute'))
    tempunits_choices = (('ftlbf', 'Feet Pound Force'),
                         ('Nm', 'Newton meter'))
    ...
    path = models.CharField(
        verbose_name='URL', max_length=400, blank=True, null=True)
    testname = models.CharField(
        verbose_name='Test Record Name', max_length=100)
    testdate = models.DateTimeField(verbose_name='Date')
    datareduced = models.BooleanField(
        default=False, verbose_name='Data Reduced')
    fileupload = models.BooleanField(
        verbose_name='File Upload', blank=True, null=True)
    hidden = models.BooleanField(blank=True, null=True)
    testdatatype = models.CharField(
        verbose_name="Test Stand", choices=testdatatype_choices, max_length=200, blank=True, null=True)
...
class ReducedPumpTestDetails(models.Model):
    pumptype_choices = (('FI', 'FI'), ('CI', 'CI'), ('KV', 'KV'), ('KS', 'KS'), ('TA', 'TA'), ('SKV', 'SKV'), ('SKS', 'SKS'), ('SFI', 'SFI'), ('SCI', 'SCI'), ('ESCC', 'End suction close-coupled'), ('ESFM',
                                                                                                                                                                                                      'End suction frame mounted/own bearings'), ('IL', 'In-Line'), ('RSV', 'Radially split, multi-stage, vertical, in-line casing diffuser'), ('ST', 'Submersible turbine'), ('Circ', 'Residential Circulator'))
    testloop_choices = (('C1', 'Commercial Front Loop'), ('C2', 'Commercial Back Loop'), (
        'R1', 'Residential Loop 1'), ('R2', 'Residential Loop 2'), ('CO', 'Commercial Outside Loop'))
    bearingframe_choices = (('H', 'H Frame'), ('J', 'J Frame'),
                            ('L', 'L Frame'), ('N', 'N Frame'), ('N/A', 'None'))
...
class ReducedPumpTestData(models.Model):
    testid = models.ForeignKey(
        ReducedPumpTestDetails, on_delete=models.CASCADE)
